...
export function ApisForm({ tenantId, settings }: ApisFormProps) {
    ...
    const cardClass = "border border-[#222222] bg-[#111111] p-6 space-y-4";
    const labelClass = "text-[10px] font-bold tracking-[0.2em] text-[#888888] uppercase block";
    const inputClass = "w-full bg-black border border-[#222222] text-white text-sm px-4 py-2 focus:outline-none focus:border-white transition-colors font-mono";

    return (
        <div className="space-y-8">
            <div className="bg-[#050505] border border-yellow-900/30 p-4 flex gap-4 items-start">
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                <div className={cardClass}>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="text-xs font-bold tracking-[0.15em] text-white uppercase">Stripe Integration</h3>
                    </div>
                ...
                <div className={cardClass}>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="text-xs font-bold tracking-[0.15em] text-white uppercase">Webhooks Externos</h3>
                    </div>
                    
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-black border border-[#222222]">
                            <div>
                                <h4 className="text-[10px] font-bold text-white tracking-widest uppercase">Activar Notificaciones HTTP</h4>
                                <p className="text-[9px] text-[#666666] mt-1 uppercase">ENVÍA EVENTOS DE CITAS Y PAGOS A TU PROPIO SERVIDOR.</p>
                            </div>
                            <button 
                                onClick={() => setConfig({...config, enableWebhooks: !config.enableWebhooks})}
                                className={`w-10 h-5 rounded-full transition-colors relative ${config.enableWebhooks ? 'bg-white' : 'bg-[#222222]'}`}
                            >
                                <div className={`absolute top-0.5 w-4 h-4 rounded-full transition-all ${config.enableWebhooks ? 'left-5.5 bg-black' : 'left-0.5 bg-[#444444]'}`} />
                            </button>
                        </div>
