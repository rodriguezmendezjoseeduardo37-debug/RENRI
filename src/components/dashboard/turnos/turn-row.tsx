...
export function TurnRow({ turn, isPast, onSkip, onCancel }: TurnRowProps) {
    ...
    return (
        <div
            className={`flex items-center justify-between p-4 border-b border-border last:border-0 ${isPast ? "opacity-50" : ""
                }`}
        >
            <div className="flex items-center gap-6">
                <span className="text-3xl font-bold text-foreground font-[family-name:var(--font-heading)] min-w-[3rem]">
                    {turn.number}
                </span>
                <div>
                    <p className="text-sm font-medium text-foreground">{turn.clientName}</p>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {onCancel && (
                        <button
                            ...
                            className="px-3 py-1.5 text-[10px] font-medium tracking-[0.15em] uppercase border border-border text-red-500 hover:border-red-400 hover:text-red-400 transition-colors"
                        >
                            CANCELAR
                        </button>
                    )}
